	...
	<c:forEach items="${returnMileageList}" var="mileageList" varStatus="status">
		<c:if test="${status.index eq 0 and sessionLevel eq 3}">
			<div class="w3-container w3-card-2 w3-white w3-round w3-margin">
				<h6 class="w3-opacity">
				사용 가능금액
					${mileageList.person.personTotalMileage}
				</h6>
			</div>
		</c:if>
		...
		<div class="w3-container w3-card-2 w3-white w3-round w3-margin">
			...
			<c:if test="${mileageList.mileageCate.mileageState == 1}">
				<img src="https://raw.githubusercontent.com/enujo/imgRepository/master/yes.png" alt="Avatar" class="w3-left w3-circle w3-margin-right" style="width: 37px">
			</c:if>
			<!-- 적립했을때 -->
			<c:if test="${mileageList.mileageCate.mileageState == 2}"> 
				<img src="https://raw.githubusercontent.com/enujo/imgRepository/master/no.png" alt="Avatar" class="w3-left w3-circle w3-margin-right" style="width: 37px">
			</c:if>
			<!-- 날짜 -->
			...
			<p>
				<c:choose>
				    <c:when test="${mileageList.mileageUsePrice == 0 }">
				        <!-- 적립시 -->
				        <h4 align="right">
				        	&#43; ${mileageList.mileageCate.mileageCatePrice}
				        </h4>
					</c:when>
				    <c:when test="${mileageList.mileageUsePrice != 0 }">
				         <!-- 사용시 -->
				         <h4 align="right">
				         	${mileageList.mileageUsePrice}
				         </h4>
