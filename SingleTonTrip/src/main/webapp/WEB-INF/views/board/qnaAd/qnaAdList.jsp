...
<c:import url="./qnaAdAdd.jsp"></c:import>
							...
							<div class="tim-container">
								<div class="card card-signup">
									<div class="header header-primary text-center"
										style="height: 80px;">
										<h4>
											<i class="material-icons">live_help</i> 광고문의내역
										</h4>
									</div>
									<div class="content">
										<div align="right">
											<c:if test="${sessionLevel ne null && sessionLevel != 1}">
												<button type="button" class="btn btn-primary"
													onclick="qnaAdAddModalShow()">
													<i class="fa fa-pencil"></i>  광고문의하기
												</button>
											</c:if>
										</div>
										<div class="table-responsive">
											...
											<div align="center">
												<c:if test="${startPage>1}">
													<a
														href="<c:url value='qnaAd?currentPage=${startPage-pageSize}'/>">prev</a>
												</c:if>
												<c:forEach var="i" begin="${startPage}" end="${endPage}"
													step="1">
													<a href="<c:url value='qnaAd?currentPage=${i}'/>">${i}</a>
												</c:forEach>
												<c:if test="${endPage ne lastPage}">
													<a
														href="<c:url value='qnaAd?currentPage=${startPage+pageSize}'/>">next</a>
												</c:if>
											</div>
										</div>
									</div>
